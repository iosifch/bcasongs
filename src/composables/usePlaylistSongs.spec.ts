...
vi.mock('./usePlaylist', () => ({
  usePlaylist: () => ({
    ...
    isInPlaylist: (id: string) => playlistRef.value.includes(id),
...
describe('usePlaylistSongs', () => {
  ...
  it('should load songs for playlist ids', async () => {
    ...
    mockGetSong.mockImplementation((id: string) => {
  ...
  it('should discard stale responses on rapid playlist changes', async () => {
    let resolveSong1: (value: unknown) => void;
  ...
  it('should call reorderPlaylist when setting playlistModel', async () => {
    ...
    playlistModel.value = [song2, song1] as never;
